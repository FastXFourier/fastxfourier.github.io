...
จะได้ว่า จะสร้างได้ก็ต่อเมื่อ $t(r_1,c_1,r_2,c_2)-t(r_1+1,c_1+1,r_2-1,c_2-1)=0$ **(นั่นคือ จำนวนต้นไม้ในพื้นที่ใหญ่ ลบด้วยพื้นที่เล็ก (กรอบรอบๆ) เท่ากับ 0)**
...
int n, m;
vector <vector <int>> can(505, vector <int> (505)), pref(505, vector <int> (505));

// คำนวณจำนวนต้นไม้ที่อยู่ในกรอบขนาด k โดยมี x,y เป็นมุมขวาล่าง
bool cal(int x, int y, int k){
    int pref1, pref2;
    pref1 = pref[x][y] - pref[x - k][y] - pref[x][y - k] + pref[x - k][y - k];
    pref2 = pref[x - 1][y - 1] - pref[x - k + 1][y - 1] - pref[x - 1][y - k + 1] + pref[x - k + 1][y - k + 1];
    return pref1 == pref2;
}
...
void solve(){
    ...
    // คำนวณ 2D Prefix Sum หรือก็แค่ p(i, j) ตามคำอธิบาย
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= m; j++) {
            pref[i][j] = can[i][j] + pref[i - 1][j] + pref[i][j - 1] - pref[i - 1][j - 1];
        }
    }
