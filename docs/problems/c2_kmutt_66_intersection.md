...
---
# ข้อสอบท้ายค่าย 2 ปีการศึกษา 2566 ศูนย์ สอวน. มหาวิทยาลัยเทคโนโลยีพระจอมเกล้าธนบุรี ข้อ [Intersection](https://grader.gchan.moe/problemset/c2_st66_intersection/) & Sweep Line Algorithm
---
...
เป็น “ช่วง” โดยใน**จุดเริ่มต้น**ของแต่ละช่วง จะมีจำนวน**จุดตัดเพิ่มมา 1** และใน**จุดจบ**ของแต่ละช่วง จะมี**จุดตัดลดลง 1**(ดังที่อธิบายไปในแนวคิดของ [Sweep Line Algorithm](#sweep-line-algorithm))

- ที่**จุดเริ่ม**ของช่วง ($min(y_i, y_{i+1})$) ให้เพิ่มค่า **+1**
  หมายถึง มีช่วงหนึ่งเริ่มต้นที่นี่
- ที่**จุดสิ้นสุด**ของช่วง ($max(y_i, y_{i+1})$) ให้ลดค่า **-1** 
  หมายถึง มีช่วงหนึ่งจบลงที่นี่

เมื่อเราทำแบบนี้กับทุกช่วงแล้ว  
เราจะสามารถรู้ได้ว่า ณ แต่ละ $y$ จะมี**การเปลี่ยนแปลงของจำนวนจุดตัด**ทั้งหมดกี่จุด  
...
ตัวแปร $sum$ จะบอกว่า “ตอนนี้มี**ส่วนของเส้นตรงกี่เส้น**ที่ยังถูกตัดอยู่” และเราก็เก็บค่า $sum$ ที่มากที่สุดที่ได้มา**ระหว่างการกวาด**
