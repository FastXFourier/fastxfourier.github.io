...
class ProblemsExtension(Extension):
    ...
    def extendMarkdown(self, md):
        ...
        md.preprocessors.register(
            ProblemsPreprocessor(md, problems_file),
            'problemlist',
            175
        )
...
class ProblemsPreprocessor(Preprocessor):
    ...
    def _generate_html(self, problems):
        ...
        all_difficulties = sorted(all_difficulties, 
                                 ...
                                 if x in ['Very Easy', 'Easy', 'Medium', 'Hard'] else 999)
...
''')
        for difficulty in all_difficulties:
            html_parts.append(f'  <label><input type="checkbox" value="{difficulty}" class="filter-difficulty"> {difficulty}</label>\n')
        ...
        for source in all_sources:
            html_parts.append(f'  <label><input type="checkbox" value="{source}" class="filter-source"> {source}</label>\n')
        ...
        for topic in all_topics:
            html_parts.append(f'  <label><input type="checkbox" value="{topic}" class="tag"> {topic}</label>\n')
        
        html_parts.append('</div>\n\n')
        
        # Problems grid
        html_parts.append('<!-- Problems grid -->\n<div class="grid cards" markdown="1">\n\n')
